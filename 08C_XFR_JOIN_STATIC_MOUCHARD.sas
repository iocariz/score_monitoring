...
proc sql;
create table	sm_tmp.sm_demand_XFR_2 as
	select		a.*,
				b.*
	from		sm_tmp.sm_demand_XFR as a
	inner join	sm_tmp.XFR_se_info_ccm_last as b
	on			a.authorization_id=b.authorization_id;	
quit;

data sm_tmp.sm_demand_XFR_3;
	length final_decision $ 50.;
	set sm_tmp.sm_demand_XFR_2(where=(status_name ne 'Pending'));
...
options mprint;	
data sm_tmp.sm_demand_XFR_4;
	length reject_reason $50.;	
	set sm_tmp.sm_demand_XFR_3;
...
data fraude_mensual fraude_acc fraude_auth;
	...
	KEEP authorization_id account_id FCH_BASE AUTORIZACION CLASIFICACION AGENTE_DETECCION 
		 ...
		 IDENTIFIED_TYPE AVOIDED_TYPE MES_CLASIFICACION_DATE MES_DETECCION_DATE 
	...
	CLASIFICACION_DATE = datepart(MES_CLASIFICACION_DATE);
	DETECCION_DATE = datepart(MES_DETECCION_DATE);
...
proc sql;
create table 	sm_demand_XFR_4f as 
	select 		a.*,	
				b.account_id_tpt
	from 		sm_tmp.sm_demand_XFR_4 as a
	left join	SM_TMP.TPT_CARD_XFR as b
	on			a.account_id=b.account_id_tj;
quit;

proc sql;
create table 	sm_demand_XFR_5f as
	select 		a.*,
				...
				coalesce(b.IDENTIFIED_TYPE, c.IDENTIFIED_TYPE, d.IDENTIFIED_TYPE) as IDENTIFIED_TYPE,
				coalesce(b.AVOIDED_TYPE, c.AVOIDED_TYPE,d.AVOIDED_TYPE) as AVOIDED_TYPE,
				coalesce(b.MES_CLASIFICACION_DATE, c.MES_CLASIFICACION_DATE, d.MES_CLASIFICACION_DATE) as MES_CLASIFICACION_DATE format=datetime20.,
				coalesce(b.MES_DETECCION_DATE, c.MES_DETECCION_DATE, d.MES_DETECCION_DATE) as MES_DETECCION_DATE format=datetime20.
	from		sm_demand_XFR_4f as a
...
data sm_demand_XFR_5frud;
	set sm_demand_XFR_5f(keep=mis_date authorization_id CLASIFICACION_DATE--MES_DETECCION_DATE);
	where CLASIFICACION_DATE ne .;
run;

proc sort data=sm_demand_XFR_5frud nodupkey;
by _all_;
run;

proc sort data=sm_demand_XFR_5frud;
by authorization_id CLASIFICACION_DATE;
run;

data sm_demand_XFR_5frud;
	set sm_demand_XFR_5frud;
	by authorization_id;
	if last.authorization_id then output;
run;

proc sql;
create table 	sm_tmp.SM_DEMAND_XFR_END as 
	select		a.*,
				...
				b.IDENTIFIED_TYPE,
				b.AVOIDED_TYPE,
				b.MES_CLASIFICACION_DATE,
				b.MES_DETECCION_DATE
	from		sm_tmp.sm_demand_XFR_4 as a
	left join	sm_demand_XFR_5frud as b
	on			a.authorization_id=b.authorization_id;
quit;

proc delete data=sm_tmp.sm_demand_XFR;run;
proc delete data=sm_tmp.sm_demand_XFR_2;run;
proc delete data=sm_tmp.sm_demand_XFR_3;run;
proc delete data=sm_tmp.sm_demand_XFR_4;run;
proc delete data=sm_demand_XFR_5frud;run;
proc delete data=sm_demand_XFR_5f;run;
proc delete data=sm_demand_XFR_4f;run;


proc freq data=sm_tmp.SM_DEMAND_XFR_END;
tables REJECT_REASON;
RUN;

DATA CHECK;
	SET sm_tmp.SM_DEMAND_XFR_END;
	KEEP authorization_id SE_Decision_id ko_score num_ko_others num_ko_score;
	WHERE KO_SCORE = 1;
RUN;