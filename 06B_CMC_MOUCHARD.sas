...
/***DWH***/
data sm_tmp.ccm_se_info_wacc(keep=SE_history_date CHG_DAY CHG_TIME se_cptestu Authorization_id &out_vars_trazas.);
 set R_ADC8.SE_ESTU_WEB_WACC(keep=&vars_wacc. 
							 where=((tipotran is null or tipotran not in ('CLIR','PAGO','PAGR','RESU','ESTR','ACOM','RESA'))
									and sociedad eq '800' and DTRT >="&init_date_se_txt.")
...
%macro renamer;
 ...
 %end;
%mend renamer; 
data sm_tmp.ccm_se_info_wacc2;
	set sm_tmp.ccm_se_info_wacc;
...
proc sql;
create table 	sm_tmp.CCM_se_info_all2  as
	select		a.*,
				b.n_calls,
				c.n_clie
	from		sm_tmp.ccm_se_info_wacc2  as a
	left join	num_calls as b
	on			a.numaut=b.numaut
	left join	num_clie as c
	on			a.numaut=c.numaut;
quit;
data sm_tmp.CCM_se_info_all2;
	set sm_tmp.CCM_se_info_all2;
	if n_clie >= 1 and n_calls > 1 and TIPOTRAN = 'CLIE' then delete;
run;
proc sort data=sm_tmp.CCM_se_info_all2 out=sm_tmp.CCM_se_info_all2_sort;
	by Authorization_id CHG_DAY CHG_TIME se_cptestu;
run;

data sm_tmp.CCM_se_info_ccm_last;
	set sm_tmp.CCM_se_info_all2_sort;
	by Authorization_id CHG_DAY CHG_TIME se_cptestu;
	if last.authorization_id;
run;

proc freq data=sm_tmp.CCM_se_info_ccm_last;
tables tipotran*red;
run;

data sm_tmp.CCM_se_info_ccm_last;
	set sm_tmp.CCM_se_info_ccm_last;
	if tipotran in ('EST1','CLIE') then VALID_CALL = 'N';
	ELSE VALID_CALL = 'Y';
run;

proc delete data=sm_tmp.CCM_se_info_all2_sort;run;
proc delete data=sm_tmp.CCM_se_info_all2;run;
proc delete data=sm_tmp.CCM_se_info;run;
proc delete data=sm_tmp.CCM_se_info_wacc2;run;
proc delete data=sm_tmp.CCM_se_info_wacc;run;


proc freq data=sm_tmp.CCM_se_info_ccm_last;
tables SE_history_date*red;
run;