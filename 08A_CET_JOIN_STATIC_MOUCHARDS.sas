...
proc sql;
create table	sm_tmp.sm_demand_2 as
	select		a.*,
				b.*
	from		sm_tmp.sm_demand as a
	inner join	sm_tmp.se_info_all_last as b
	on			a.authorization_id=b.authorization_id;	
quit;

data sm_tmp.sm_demand_3;
	length final_decision $ 50.;
	set sm_tmp.sm_demand_2(where=(status_name ne 'Pending'));
...
proc sql ;
	...
	(index(upcase(descripcion), 'PERFIL RIESGO') > 0 and substr(compress(descripcion), 1, 1) = "*" and origen = 'T') or
	(index(upcase(descripcion), 'PERFIL SCORE') > 0 and substr(compress(descripcion), 1, 1) = "*" and origen = 'T')
; 
quit;
...
options mprint;	
data sm_tmp.sm_demand_4;
	length reject_reason $50.;	
	set sm_tmp.sm_demand_3;;
	...
	%iterSpecificAlert(type = ko_fraude); /*6*/
...
proc delete data=sm_tmp.sm_demand;run;
proc delete data=sm_tmp.sm_demand_2;run;
proc delete data=sm_tmp.sm_demand_3;run;
proc delete data=sm_tmp.sm_demand_4;run;
