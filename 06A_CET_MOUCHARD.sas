...
/***DWH***/
data sm_tmp.se_info_wacc(keep=SE_history_date CHG_DAY CHG_TIME se_cptestu Authorization_id &out_vars_trazas.);
 set R_ora4.SE_ESTU_WEB_WACC(keep=&vars_wacc. 
							 where=((tipotran is null or tipotran not in ('CLIR','PAGO','PAGR','RESU','ESTR','ACOM','RESA'))
									and sociedad eq '400' and DTRT >="&init_date_se_txt.")
...
%macro renamer;
 ...
 %end;
%mend renamer; 
data sm_tmp.se_info_wacc2;
	set sm_tmp.se_info_wacc;
	...
	%renamer;
run; 
data sm_tmp.se_info_all2;
		set sm_tmp.se_info_wacc2;
run;
proc sql;
create table num_calls as
	select numaut, count(*) as n_calls
	from sm_tmp.se_info_all2
	group by 1;
quit;
proc sql;
...
proc sql;
create table 	sm_tmp.se_info_all2  as
	select		a.*,
				b.n_calls,
				c.n_clie
	from		sm_tmp.se_info_all2  as a
...
data sm_tmp.se_info_all2;
	set sm_tmp.se_info_all2;
	if n_clie >= 1 and n_calls > 1 and TIPOTRAN = 'CLIE' then delete;
run;

proc sort data=sm_tmp.se_info_all2 out=sm_tmp.se_info_all2_sort;
	by Authorization_id CHG_DAY CHG_TIME se_cptestu;
run;
data sm_tmp.se_info_all_last;
	set sm_tmp.se_info_all2_sort;
	by Authorization_id CHG_DAY CHG_TIME se_cptestu;
	if last.authorization_id;
run;

data sm_tmp.se_info_all_last;
	set sm_tmp.se_info_all_last;
	if tipotran in ('EST1','CLIE','ESTU1') then VALID_CALL = 'N';
	ELSE VALID_CALL = 'Y';
run;

proc delete data=sm_tmp.se_info_all2_sort;run;
proc delete data=sm_tmp.se_info_all2;run;
proc delete data=sm_tmp.se_info;run;
proc delete data=sm_tmp.se_info_wacc2;run;
proc delete data=sm_tmp.se_info_wacc;run;

data check;
	set sm_tmp.se_info_all_last;
	keep Authorization_id dismaxtj;
	where authorization_id = 4107210466;
run;